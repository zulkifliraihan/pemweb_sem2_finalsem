...
<?php
	include './application/views/dashboard/admin/merk/modal.php';
	...
	$(document).on('click', '#edit-data', function () {
		...
		$.ajax({
			...
			success: function (response) {
				...
				let merk = response.data;

				let formAction = $('#edit-data-form').attr('action');
				formAction = formAction.replace(':did', merk.id);
				$('#edit-data-form').attr('action', formAction);

				$('#nama-edit').val(merk.nama);
				$('#produk-edit').val(merk.produk);
